...
let goToLogin = () => { // 浏览器跳转在这个版本需要去掉
    let redirectUrl;
    // if (isWeixinBrowser()) { // 是微信内置浏览器，或qq内置浏览器
    //     console.log('[是微信内置浏览器，或qq内置浏览器]');
    //     redirectUrl = queryParam.setQueryParam(env.static.wechat, {
    //         r: window.location.href,
    //         loginUrl: env.static.account
    //     });
    // } else {
    //     console.log('[普通浏览器]');
    //     redirectUrl = queryParam.setQueryParam(env.static.login, {
    //         r: window.location.href
    //     });
    // }
    redirectUrl = queryParam.setQueryParam(env.static.login, {
